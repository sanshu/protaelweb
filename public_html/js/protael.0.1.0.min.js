...
l=e/(f-g),m,q=this.protael.W,n=this.paper;console.log(f);console.log(g);for(c=0;c<=d;c++);!a.values&&a.data&&(a.values=0<a.data.indexOf(",")?a.data.split(","):a.data.split(""));a.values.splice(0,0,0);for(d=a.values[d]=0;d<q;d++)c=d,m=b+e-(a.values[d]-g)*l,d&&NaN!==d&&(p=p+"L"+c+", "+m),d||(p=p+"M0 "+(b+e+g*l));Array.isArray(j)&&(1==j.length?h=j[0]:2==j.length?h=n.gradient("l(0, 1, 0, 0)"+j[0]+"-"+j[1]):3==j.length&&(h=n.gradient("l(0, 1, 0, 0)"+j[0]+"-"+j[1]+":"+k+"-"+j[2])));j=n.path(p+"L"+q+" "+
(b+e+g*l)+" Z").attr({stroke:j,"stroke-width":0.1,"stroke-linejoin":"round","stroke-linecap":"butt","stroke-dasharray":"",fill:h,opacity:".7"});h=n.text(0.1,b+8,a.label).attr({"font-size":"10px","text-anchor":"start"});g=n.line(0,b,q,b).attr({"class":"pl-chart-top"});f=n.line(0,b+f*l,q,b+f*l).attr({"class":"pl-chart-center"});b=n.line(0,b+e,q,b+e).attr({"class":"pl-chart-bottom"});a=n.g(j,g,b,f,h).attr({id:"qtrack_"+a.label});this.gQTracks.add(a);this.viewSet.push(g);this.viewSet.push(b);this.viewSet.push(f);
