...
var Protael = (function() {
    ...
    function createToolbarBtns(r, toolbar, showControls) {
        ...
        toolbar.append($('<button>Zoom to fit</button>').button({
            text: false,
            icons: {
        ...
        toolbar.append($('<button>Zoom to selection</button>').button(
                {
                    text: false,
                    icons: {
        ...
        toolbar
                .append('Current selection: <input type="text" class="protael_selection_inp" readonly/>');
        toolbar.append($('<button id="export">Export</button>').button({
            text: false,
            icons: {
        ...
        toolbar.append($('<button>Reset selection</button>').button({
            text: false,
            icons: {
        ...
        toolbar
                .append($(
                        ...
                        .change(
                                function() {
                                    r.setShowCursorTooltips($("#chkTooltip").is(
                                            ':checked'));
                                }));
...
//
    ...
    (function(paperproto) {
        ...
        paperproto.setZoom = function(zoom) {
            ...
            this.setWidth(Math.round(newWidth));
            ...
            if (this.isChrome) {
                ...
                var unstrW = this.strechableSeq.getBBox().width,
                        deltaW = newWidth - unstrW,
                        sl = this.strechableSeq.attr("numspaces"),
                        newspa = deltaW / sl + "px",
                        x = this.strechableSeq.attr("x");
                this.strechableSeq.attr({
                    x: x * ds,
            ...
            p.currentScaleX = zoom;
            p.currentScaleX > 1 ? this.seqBGSet.show() : this.seqBGSet.hide();
            p.currentScaleX > 7 ? this.seqChars.show() : this.seqChars.hide();
            p.currentScaleX > 7 ? this.overlayFtLabels.forEach(function(t) {
                t.hide();
        ...
        paperproto.proteinSequence = function(chars, y, showSequence, alignment) {
            ...
            if (showSequence) {
                ...
                if (this.isChrome) {
                    this.strechableSeq = this.paper.text(startX * inst.currentScale(), y + 8,
                            chars.join(''));
                    unstrW = this.strechableSeq.getBBox().width;
                    this.strechableSeq.attr({
                        'uwidth': unstrW,
                        'numspaces': chars.length
                    });
                    this.textSet.push(this.strechableSeq);
                    this.seqChars.add(this.strechableSeq);
                }
                else {
                    setTimeout(function() {
                        if (showSequence) {
...
//TODO: micro-opt
        ...
        paperproto.draw = function(protein) {
            ...
            if (this.protael.isChrome) {
                this.strechableSeq = this.paper.text(0, 30, protein.sequence).attr({
                    id: "strechableSeq",
                    'font-family': 'monospace',
                    'font-size': "9px",
                    "text-anchor": "start",
                ...
                this.strechableSeq.attr({
                    'uwidth': this.strechableSeq.getBBox().width,
                    'numspaces': chars.length
                });
            }
            // ///////////////////////////
            ...
            if (protein.alignments) {
                ...
                for (i = 0; i < counterMax; i++) {
                    ...
                    var schars = ali.sequence.split('');

                    this.proteinSequence(schars, topY, show, ali);
//                    this.proteinSequence(ali.sequence.split(''),
//                        topY, showAlignments,
//                        ali.description,
//                        ali.start || 0);
                    topY += 10;
            ...
            paper.mousemove(function(e) {
                ...
                var x = Math.round(xoff + parent.currentShiftX);
                if (x < 0)
                    return;
                self.pointer.attr({
                    'x': x + delta
    ...
    Protael.prototype.Utils.calcHeight = function(protein) {
        var h = uiOptions.mainSequenceHeight, // main sequence
                y = 0;
    ...
    (function(protaelproto) {
        ...
        protaelproto.setSelection = function(minx, maxx) {
            ...
            this.paper.selector.attr({
                'width': wd,
                'x': this.toScreenX(minx - 1) + this.currentShiftX
            }).show();
            if (this.controlsEnabled) {
        ...
        protaelproto.zoomIn = function() {
            this.setZoom(this.currentScaleX + 0.1);
            return this;
        };
        ...
        protaelproto.zoomToSelection = function() {
            ...
            var s = (this.selectedx[0] + w / 2) * this.currentScaleX - this.svgDiv.width() / 2;
        ...
        protaelproto.toOriginalX = function(x) {
            return Math.round((x + this.currentShiftX) / this.currentScaleX);
        };
        protaelproto.toScreenX = function(x) {
            return Math.round(x * this.currentScaleX - this.currentShiftX);
        };
        ...
        protaelproto.setZoom = function(zoom) {
            ...
            $("#" + this.container + ' .protael_zoomslider').slider("value", this.currentScaleX);
            return this;
        };
